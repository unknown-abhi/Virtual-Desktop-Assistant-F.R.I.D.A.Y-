...
def test():
    ...
    print ('according to my analysis you are feeling')
    for x in emotion_list:
        speak(x)
        if 'depress' in x or 'sad' in x:
            print ('Text_Notification.py')
            exec(open('text_notify.py').read())
        print (x)
        file = open('test.txt', 'r+')
        file.truncate(0)
        file.close()
        emotion_list.truncate(0)
